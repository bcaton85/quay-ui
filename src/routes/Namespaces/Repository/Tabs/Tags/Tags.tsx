...
import {Tag,TagsResponse,getTags, getManifestByDigest,getSecurityDetails, ManifestByDigestResponse, ManifestList, SecurityDetailsResponse} from 'src/resources/TagResource';
    ...
    useEffect(() => {
        ...
        (async () => {
            const getManifest = async (tag: Tag) => {
                let manifestResp: ManifestByDigestResponse = await getManifestByDigest(props.organization, props.repository, tag.manifest_digest)
            ...
            do {
                try{
                    const resp: TagsResponse = await getTags(props.organization, props.repository, page);
                    await Promise.all(resp.tags.map((tag: Tag) => tag.is_manifest_list ? getManifest(tag) : null));
