...
import { Tag, TagsResponse, getTags, getManifestByDigest, getSecurityDetails } from 'src/resources/TagResource';
...
var createTagResponse = (): TagsResponse => {
    return {
        page: 1,
...
test('Tags should render', async () => {
    ...
    render(<RecoilRoot><Tags organization={testOrg} repository={testRepo} /></RecoilRoot>);
    const message = await screen.findByText('This repository is empty.');
    expect(message).toBeTruthy();
});

test('Tags should appear in list', async () => {
    let mockResponse = createTagResponse()
...
test('Updating per page should update table content', async () => {
    let mockResponse: TagsResponse = createTagResponse();
    for(let i=0;i<40;i++){
        mockResponse.tags.push(createTag(`tag${i}`))
    }
    mocked(getTags, true).mockResolvedValue(mockResponse);
    const {container} = render(<RecoilRoot><Tags organization={testOrg} repository={testRepo} /></RecoilRoot>);
    const initialRows = await screen.findAllByTestId('table-entry');
    expect(initialRows.length).toBe(25);
    let perPageButton = (await screen.findByTestId('pagination')).querySelector('button') // Assumes 1st button is per page
    fireEvent(perPageButton, new MouseEvent('click', { bubbles: true, cancelable: true, }), );
    fireEvent(await screen.findByText('10 per page'), new MouseEvent('click', { bubbles: true, cancelable: true, }), );
...
test('Updating page should update table content', async () => {
    ...
    const initialRows = await screen.findAllByTestId('table-entry');
    expect(initialRows.length).toBe(25);
    fireEvent(screen.getByLabelText('Go to next page'), new MouseEvent('click', { bubbles: true, cancelable: true, }), );
    const updatedRows = screen.getAllByTestId('table-entry');
...
test('Copy modal should show org and repo', async () => {
    ...
    fireEvent(screen.getByTestId('pull'), new MouseEvent('click', { bubbles: true, cancelable: true, }), );
    expect(screen.getByTestId(`copy-tag`).querySelector("input").value).toBe(`docker pull quay.io/${testOrg}/${testRepo}:${tag.name}`)
    expect(screen.getByTestId(`copy-digest`).querySelector("input").value).toBe(`docker pull quay.io/${testOrg}/${testRepo}@${tag.manifest_digest}`)
});
