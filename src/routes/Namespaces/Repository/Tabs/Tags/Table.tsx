  ...
  return (
    <>
      ...
      >
        ...
        <ClipboardCopy data-testid="copy-tag" isReadOnly hoverTip="Copy" clickTip="Copied">
        ...
        {/* TODO: Pull in repo name */}
        <ClipboardCopy data-testid="copy-digest" isReadOnly hoverTip="Copy" clickTip="Copied">
      ...
      <TableComposable aria-label="Expandable table">
        ...
        {props.tags.map((tag: Tag, rowIndex: number) => {
          ...
          return (
            <Tbody data-testid="table-entry" key={tag.name} isExpanded={isTagExpanded(tag)}>
              <Tr>
                <Td
                  expand={
                ...
                <Td dataLabel={columnNames.OS}>os-mock</Td> 
                 <Td dataLabel={columnNames.Security}>{typeof (tag.manifest_list) != 'undefined' ? "See Child Manifest" : <SecurityDetails org={props.organization} repo={props.repository} digest={tag.manifest_digest} />}</Td>
                <Td dataLabel={columnNames.Size}>{typeof (tag.manifest_list) != 'undefined' ? "N/A" : tag.size}</Td>
                <Td dataLabel={columnNames.LastModified}>{tag.last_modified}</Td>
                <Td dataLabel={columnNames.Expires}>expires-mock</Td>
                <Td dataLabel={columnNames.Manifest}>{tag.manifest_digest.substring(0, 19)}</Td>
                <Td dataLabel={columnNames.Pull}><i onClick={() => { openModal(tag) }} data-testid="pull" className="fa fa-download"></i></Td>
              </Tr>
              {tag.manifest_list ? tag.manifest_list.manifests.map((manifest, rowIndex) => {
                return (
                  <Tr key={rowIndex} isExpanded={isTagExpanded(tag)}>
                    <Td />
                    <Td />
                    {manifest.platform ? (
                      <Td dataLabel="platform" noPadding={childHasNoPadding} colSpan={archColspan}>
                        <ExpandableRowContent>{`${manifest.platform.os} on ${manifest.platform.architecture}`}</ExpandableRowContent>
                      </Td>
                    ) : <Td />}
                    <Td dataLabel="security" noPadding={childHasNoPadding} colSpan={securityColspan}>
                      <ExpandableRowContent><SecurityDetails org={props.organization} repo={props.repository} digest={manifest.digest} /></ExpandableRowContent>
                    </Td>
                    {manifest.size ? (
                      <Td dataLabel="size" noPadding={childHasNoPadding} colSpan={sizeColspan}>
                        <ExpandableRowContent>{manifest.size}</ExpandableRowContent>
                      </Td>
                    ) : <Td />}
                    {manifest.digest ? (
                      <Td dataLabel="digest" noPadding={childHasNoPadding} colSpan={manifestColspan}>
                        <ExpandableRowContent>{manifest.digest.substring(0, 19)}</ExpandableRowContent>
                      </Td>
                    ) : <Td />}
                  </Tr>
                )
